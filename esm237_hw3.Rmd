...
data <- raw_data %>%
  ...
  mutate(t_min_c = t_min_k-273, 
         ...
         # calculate humidex
         hx_min = hx(t_min_c, v_min),
         hx_max = hx(t_max_c, v_max),
...
ggplot(data = at_40) +
  geom_point(aes(x=year(time), y=at_max), color="orangered3") +
  geom_smooth(aes(x=year(time), y=at_max),method = "lm", color = "darkolivegreen") +
  geom_point(aes(x=year(time), y=at_min), color="orange") +
  geom_smooth(aes(x=year(time), y=at_min),method = "lm", color = "darkolivegreen") +
  labs(x = "Year",
       y = "Days per year",
       title = "Projected AT",
       subtitle = "Los Angeles, 2006 to 2100",
